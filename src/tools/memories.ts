...
/**
 * Create memory tools with userId context
 */
...
Returns summaries of relevant past conversations.`,
    ...
    execute: async ({ query, limit }) => {
      try {
        ...
        const results = await searchConversations(userId, query, searchLimit);
        ...
        const memories = await Promise.all(
          results.map(async result => {
            ...
            const fullConversation = await getConversation(result.id, userId);
...
Use this when you need more context from a conversation that was found with recallMemories.`,
    ...
    execute: async ({ title }) => {
      try {
        ...
        const results = await searchConversations(userId, title, 1);
  ...
  return { recallMemories, getMemoryDetails };
}
